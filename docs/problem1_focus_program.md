...
1. Prove that the actual shift-zero schedule-survivor pair stream generated by
   `G(11), G <- G|_11 p q` is not eventually `00`.
2. Seek a closure theorem or cocycle for the coupled moving-fringe generator
   and word state `G`; finite pair prefixes are regression evidence only.
3. Classify an eventually-`00` output cycle only after proving a
   depth-independent state quotient or another exact closure mechanism.
4. If the coupled transducer does not close, transfer the mismatch valuation
   to an original-spacetime finite-support obstruction rather than enlarging
   fixed portraits.
5. Formalize the contraction, survivor uniqueness, moving-tail identity, and
   mismatch law only when they enter a genuine infinite argument.
