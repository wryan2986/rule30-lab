#!/usr/bin/env bash
# .astra_dash_status.sh - Compact full-width status dashboard
...
while true; do
  ...
  # ── Read mode and totals from metadata (authoritative) ──
  ...
  if [ "$no_supervisor" = "true" ]; then
    # No supervisor running — show elapsed from last known start or nothing
    total_display="--"
    remaining_str="--"
    ...
    # Fallback: no metadata but supervisor is running (legacy mode)
    total_display="8h"
    if [ "$sup_epoch" -gt 0 ]; then
      # Legacy: assume 8h default
  ...
  if [ -f "$USAGE_FILE" ]; then
    raw=$(grep "gpt-6-astra" "$USAGE_FILE" 2>/dev/null | jq -r "select(.status==200) | .usage.inputTokens // 0" 2>/dev/null | tail -1) || true
  ...
  if [ "$no_supervisor" = "true" ]; then
    ...
    printf "%b  Round %s \u2022 %s    Total %s    ETA ~%s\n" \
      "$sc" "${codex_round:--}" "$(fmt_duration "$rnd_el")" \
      "$total_display" "$remaining_str"
  fi
  ...
  # -- Rollover status --
  rollover_state=$(cat /tmp/astra-supervisor/rollover_state 2>/dev/null || echo NORMAL)
  ...
  case "$rollover_state" in
    NORMAL)
      rollover_display="${GRN}NORMAL${RST}"
      ;;
    WRAP_REQUESTED)
      rollover_display="${YEL}WRAP REQUESTED${RST}"
      ;;
    ROLLOVER_REQUESTED)
      rollover_display="${RED}ROLLOVER REQUESTED${RST}"
      ;;
    HARD_INTERRUPT)
      rollover_display="${RED}${BOLD}HARD INTERRUPT${RST}"
      ;;
    EMERGENCY_KILL)
      rollover_display="${RED}${BOLD}FORCED ROLLOVER${RST}"
      ;;
    *)
